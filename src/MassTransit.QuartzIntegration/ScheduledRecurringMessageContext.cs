...
{
    public class ScheduledRecurringMessageContext :
        MessageContext,
        ISendContext
    {
         string _body;

        public ScheduledRecurringMessageContext(string body)
        {
            _body = body;
            Id = NewId.NextGuid();
            DeclaringMessageType = typeof(ScheduledRecurringMessageContext);
        }
        ...
        public void SetBodyHeaders(IDictionary<string, string> headers)
        {
            JObject bodyJObject = JObject.Parse(_body);
            bodyJObject["headers"] = JToken.FromObject(headers);
            _body = JsonConvert.SerializeObject(bodyJObject, Formatting.Indented);
        }
    }
}