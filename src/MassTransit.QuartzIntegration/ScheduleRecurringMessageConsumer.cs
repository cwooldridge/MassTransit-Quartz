...
namespace ServiceBus.RuntimeServices
{
    ...
    public class ScheduleRecurringMessageConsumer : Consumes<IScheduleRecurringMessage>.Context
    {
        static readonly ILog Log = Logger.Get<ScheduleRecurringMessageConsumer>();
        readonly IScheduler _scheduler;

        public ScheduleRecurringMessageConsumer(IScheduler scheduler)
        {
            _scheduler = scheduler;
        }

        public void Consume(IConsumeContext<IScheduleRecurringMessage> context)
        {
            if (Log.IsDebugEnabled)
            {
                Log.DebugFormat("ScheduleRecurringMessage: {0} at {1}", context.Message.CorrelationId,
                    context.Message);
            }

            var jobDetail = CreateJobDetail(context);

            _scheduler.AddJob(jobDetail,true);


            var triggerKey = new TriggerKey("Trigger For " + context.Message.ScheduleId, context.Message.ScheduleGroup);

            var trigger = CreateTrigger(context.Message, jobDetail, triggerKey);

            var hs =  new Quartz.Collection.HashSet<ITrigger> {trigger};
        ...
        public static IJobDetail CreateJobDetail(IConsumeContext<IScheduleRecurringMessage> context)
        {
            string body;
            ...
            var jobBuilder = JobBuilder.Create<ScheduledRecurringMessageJob>()
                .RequestRecovery(true)
                .WithIdentity(context.Message.ScheduleId, context.Message.ScheduleGroup)
                ...
                .UsingJobData("ExpirationTime",
                    context.ExpirationTime.HasValue
                        ? context.ExpirationTime.Value.ToString(CultureInfo.InvariantCulture)
                        : "")
                .UsingJobData("Network", context.Network)
                .UsingJobData("RetryCount", context.RetryCount).StoreDurably(true);

            IJobDetail jobDetail = jobBuilder.Build();
            return jobDetail;
        }

        public static ITrigger CreateTrigger(IRecurringSchedule message, IJobDetail jobDetail, TriggerKey triggerKey)
        {
            var tz = TimeZoneInfo.Local;
            if (message.TimeZoneId.IsNotEmpty() && message.TimeZoneId != tz.Id)
            {
            ...
            var triggerBuilder = TriggerBuilder.Create()
                ...
                .WithCronSchedule(message.CronExpression, x =>
                {
                    ...
                    {
                        case Lopezfoods.Scheduling.MisfireInstruction.DoNothing:
                            x.WithMisfireHandlingInstructionDoNothing();
                            break;
                        case Lopezfoods.Scheduling.MisfireInstruction.FireOnceNow:
